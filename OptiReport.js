...
// choose an appropriate step size based on range (seconds)
function getStep(start, end) {
    const amount = end - start;
    const day = 24 * 3600;
    if (amount >= 30 * day) return 30 * 60;
    if (amount >= 7 * day)  return 5 * 60;
    if (amount >= 2 * day)  return 2 * 60;
    if (amount >= 1 * day)  return 1 * 60;
    if (amount >= 12 * 3600) return 30;
    return 15;
}


//https://foundryoptifleet.com/api/KeyMetrics?siteId=6&subcustomerId=334&interval=300&step=300&start=1748832330&end=1750473781&withEvents=false&userId=9d0f7cb1-74ef-4fd1-bc86-f043d51e9432&companyFilter=331
const siteId = 6
const subcustomerIdFullSite = null;
const subcustomerIdBitmain = 334;
const subcustomerIdFortitude = 353;
const subcustomerIdRAMM = 335;
const companyFilter = 331;

function fetchKeyMetricsAverageData(subcustomerId, start, end, uptimeCell) {

    // convert start/end to unix timestamp if in Date(); format
    ...
    console.log(`Fetching KeyMetrics data for subcustomerId`);
    ...
    const url = 'https://foundryoptifleet.com/api/KeyMetrics';
    ...
    return new Promise((resolve, reject) => {
        GM_xmlhttpRequest({
            ...
            onload(res) {
                if (res.status !== 200) {
                    console.error(`Error fetching KeyMetrics data: ${res.statusText}`);
                    reject(new Error(`HTTP error! status: ${res.status}`));
                    return;
                }

                let responseData = res.response[0];
                let averageUptime = responseData.averageUptime;
                if( uptimeCell !== null && uptimeCell !== undefined) {
                    uptimeCell.textContent = averageUptime + '%';
                }
...
window.addEventListener('load', function () {
    ...
    // --- Function to enable mouse inputs in a given window ---
    function enableMouseInputsInWindow(targetWindow) {
        if (targetWindow && targetWindow.document) {
            const styleId = 'opti-report-pointer-blocker';
            const styleElement = targetWindow.document.getElementById(styleId);
            if (styleElement) {
                styleElement.remove();
                console.log('[Opti-Report] Removed pointer blocker style from target window.');
            } else {
                console.warn('[Opti-Report] Pointer blocker style not found in target window.');
            }
        } else {
            console.warn('[Opti-Report] Could not enable mouse inputs: target window or its document is not available.');
        }
    }
    ...
    function parseReportDate(dateString) {
        ...
        // Try to match "MM-DD-YYYY" (no time)
        parts = dateString.match(/(\d{2})-(\d{2})-(\d{4})/);
        if (parts) {
            // parts[1]=MM, parts[2]=DD, parts[3]=YYYY
            return new Date(parts[3], parseInt(parts[1], 10) - 1, parts[2]);
        }
    ...
    async function captureSingleReport(reportWindow, dateRangeKey, desiredRangeTextInSpan, emailBody, updateProgress, selectFortitude) {
            ...
            while (elapsedTime < maxTotalWaitTime && !dateRangeTextConfirmed) {
                ...
                try {
                    ...
                    if(dateRangeKey === "Last 30 Days") {
                        ...
                        currentRangeSpan = reportRangeDiv.querySelector('span.text');
                        let newRangeText = currentRangeSpan ? currentRangeSpan.textContent : '';
                        // May 1, 2025 3:15am - May 31, 2025 3:00am
                        const dateParts = newRangeText.split(' - ');
                        if (dateParts.length === 2) {
                            let dayStart = dateParts[0].trim().split(' ')[1];
                            let dayEnd = dateParts[1].trim().split(' ')[1];
                            // Conver to number only
                            dayStart = parseInt(dayStart, 10);
                            dayEnd = parseInt(dayEnd, 10);
                            let currentDayInt = new Date().getDate();
                            if(dayStart === 1 && dayEnd === currentDayInt) {
                                dateRangeTextConfirmed = true;
                                updateProgress(`Date range text confirmed: "${desiredRangeTextInSpan}".`);
                            }
                        } else {
                            console.log('[Opti-Report] Unexpected date format in selectedDate:', newRangeText);
                        }
        ...
        if (dateRangeTextConfirmed) {
            ...
            } else if (dateRangeKey === "Last 30 Days") {

                // Get difference between dateStart and dateEnd
                if (!dateStart || !dateEnd) {
                    console.error('[Opti-Report] dateStart or dateEnd is not set for Month to Day. Cannot verify chart.');
                    throw new Error('dateStart or dateEnd is not set for Month to Day. Cannot verify chart.');
                }
                const startDate = dateStart;
                const endDate = dateEnd;
                if (!startDate || !endDate) {
                    console.error('[Opti-Report] Could not parse dateStart or dateEnd for Month to Day.');
                    throw new Error('Could not parse dateStart or dateEnd for Month to Day.');
                }
                const diffHours = getDateDifferenceInHours(startDate, endDate);
                console.log(`[Opti-Report] Calculated date difference for Month to Day: ${diffHours} hours.`);
                if (diffHours === null) {
                    console.error('[Opti-Report] Could not calculate date difference for Month to Day.');
                    throw new Error('Could not calculate date difference for Month to Day.');
                }

                expectedMinHours = diffHours - 4; // Allowing a 4-hour tolerance
                expectedMaxHours = diffHours + 4; // Allowing a 4-hour tolerance
            } else {
    ...
    async function fetchKeyMetricsReportScreenshot(fullReportEmailBody, fortitudeReportEmailBody) {
        ...
        metricsReportWindow.onload = async () => {
            try {
                ...
                updateProgressMessage('Report page loaded.');
                ...
                */
                ...
                try {
                    ...
                    let allEmails = [fortitudeReportEmailBody];
                    allEmails.forEach((emailBodyToUpdate, index) => {
                        const tables = emailBodyToUpdate.getElementsByTagName('table');
                        if (tables.length > 0) {
                            const targetTable = tables[0]; // Assuming the first table is for Fortitude and the second for Full Report
                            const rows = targetTable.getElementsByTagName('tr');
                            for (let i = 0; i < rows.length; i++) {
                                const cells = rows[i].getElementsByTagName('td');
                                if (cells.length > 1 && cells[0].textContent === "Uptime 24hr Average") {
                ...
                // Add title and separator for the 30-day report
                const title30DayHTML = `<p style="font-size: 16px; color: #fff; margin-top: 25px; margin-bottom: 10px; font-weight: bold;">Month to Date Average</p>`;
                emailBodiesArray.forEach(emailBodyToAppendTo => {
                ...
                if (emailBodiesArray[0]) { // Only for Full Report
                    ...
                    // Find the "24 Hour Average" title element
                    const allParagraphs = emailBodyToAppendTo.getElementsByTagName('p');
                    let sevenDayTitleElement = null;
                    for (const p of allParagraphs) {
                        if (p.textContent.trim() === "24 Hour Average") {
                            sevenDayTitleElement = p;
                            break;
                        }
                    }

                    if (sevenDayTitleElement) {
                        ...
                        while(tempDivSiteOverview.firstChild) {
                            sevenDayTitleElement.parentNode.insertBefore(tempDivSiteOverview.firstChild, sevenDayTitleElement);
                        }
                    } else {
                        throw new Error('Could not find "24 Hour Average" title to insert Site Overview before it.');
                    }
                }
                ...
                if (emailBodiesArray[0]) { // Only for Full Report
                    ...
                    // Find the "Site Overview" title element that was previously inserted
                    const allParagraphs = emailBodyToAppendTo.getElementsByTagName('p');
                    let siteOverviewTitleElement = null;
                    for (const p of allParagraphs) {
                        if (p.textContent.trim() === "Site Overview") {
                            siteOverviewTitleElement = p;
                            break;
                        }
                    }

                    if (siteOverviewTitleElement) {
                        // The separator image should be the next element
                        const separatorElement = siteOverviewTitleElement.nextElementSibling;
                        if (separatorElement && separatorElement.tagName === 'IMG') {
                            // Insert the Site Overview screenshot with spacers after the separator
                            const p1 = document.createElement('p');
                            separatorElement.parentNode.insertBefore(p1, separatorElement.nextSibling);
                            p1.parentNode.insertBefore(imgElementSO, p1.nextSibling);
                            const p2 = document.createElement('p');
                            imgElementSO.parentNode.insertBefore(p2, imgElementSO.nextSibling);
                            emailBodyToAppendTo.scrollTop = emailBodyToAppendTo.scrollHeight;
                        } else {
                             throw new Error('Could not find the separator for "Site Overview".');
                        }
                    } else {
                        // This should not happen if the title was inserted correctly
                        throw new Error('Could not find "Site Overview" title to insert the screenshot.');
                    }
                }
                ...
                const titleUptimeStatsHTML = `<p><br></p>` + `<p style="font-size: 16px; color: #fff; margin-top: 25px; margin-bottom: 10px; font-weight: bold;">Uptime</p>`;
                if (emailBodiesArray[0]) { // Only for Full Report
                    const emailBodyToAppendTo = emailBodiesArray[0];
                ...
                if (!upTimeGrid) {
                    throw new Error(`Could not find Uptime grid "${uptimeGridSelector}" after clicking the tab.`);
                }
                ...
                while (zoneUptimeStatsPanels.length === 0) {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 500));
                    zoneUptimeStatsPanels = upTimeGrid.querySelectorAll('a.stat-panel');
                    if (cancelMetricsFetchFlag) throw new Error('Operation cancelled by user.');
                }
                ...
                if(!images_only) {
                    const weatherReportsTitle = `<p><br></p>` + `<p style="font-size: 16px; color: #fff; margin-top: 25px; margin-bottom: 10px; font-weight: bold;">Notes From Your Site Manager</p>`;
                    emailBodiesArray.forEach(emailBodyToAppendTo => {
    ...
    async function generateReportContent(reportType, container) {
        ...
        <table style="${tableStyle}">
            ...
            <tr><td style="${thTdStyle}"><b>Miners Out For Repair</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
        ...
        if (reportType === "Full Report") {
            ...
            <table style="${tableStyle}">
                ...
                <tr><td style="${thTdStyle}"><b>Uptime 24hr Average</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Uptime Monthly Average</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Site Utilization</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Efficiency</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
            </table>`;
            ...
            const bitmainTitleHTML = `<p style="${sectionTitleStyle}">Bitmain Information</p>` + imageSeparatorHTML;
            ...
            <table style="${tableStyle}">
                ...
                <tr><td style="${thTdStyle}"><b>Efficiency</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Miners Offline Awaiting Shipment</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Miners Shipped For Repair</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Spare Scrap</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Spare Miners</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
                <tr><td style="${thTdStyle}"><b>Total Offline</b></td><td style="${thTdStyle}">${placeholderData}</td></tr>
            </table>`;
            ...
            start24hr.setHours(start24hr.getHours() - 24);
            const end24hr = new Date();
            const fortitude24hrCell = fortitudeTable.querySelector('tr:nth-child(4) td:nth-child(2)');
            const ramm24hrCell = rammTable.querySelector('tr:nth-child(4) td:nth-child(2)');
            const bitmain24hrCell = bitmainTable.querySelector('tr:nth-child(4) td:nth-child(2)');
            fetchKeyMetricsAverageData(subcustomerIdFortitude, start24hr, end24hr, fortitude24hrCell);
            fetchKeyMetricsAverageData(subcustomerIdRAMM, start24hr, end24hr, ramm24hrCell);
            fetchKeyMetricsAverageData(subcustomerIdBitmain, start24hr, end24hr, bitmain24hrCell);

            const monthStart = new Date();
            monthStart.setDate(1);
            const monthEnd = new Date();
            const fortitudeMonthCell = fortitudeTable.querySelector('tr:nth-child(5) td:nth-child(2)');
            const rammMonthCell = rammTable.querySelector('tr:nth-child(5) td:nth-child(2)');
            const bitmainMonthCell = bitmainTable.querySelector('tr:nth-child(5) td:nth-child(2)');
            fetchKeyMetricsAverageData(subcustomerIdFortitude, monthStart, monthEnd, fortitudeMonthCell);
            fetchKeyMetricsAverageData(subcustomerIdRAMM, monthStart, monthEnd, rammMonthCell);
            fetchKeyMetricsAverageData(subcustomerIdBitmain, monthStart, monthEnd, bitmainMonthCell);

            getMinerData(function(minerData) {
                console.log('Miner data fetched for Full Report:', minerData);
                let subcustomerStats = {
                    Fortitude: { online: 0, offline: 0, total: 1120, hashrate: 0, expectedHashRate: 0 },
                    RAMM: { online: 0, offline: 0, total: 1030, hashrate: 0, expectedHashRate: 0 },
                    Bitmain: { online: 0, offline: 0, total: 8400, hashrate: 0, expectedHashRate: 0 }
                };
                let totalOnline = 0;
                let totalOffline = 0;
                    ...
                    for (const subcustomer in subcustomerStats) {
                        if (miner.subcustomerName && miner.subcustomerName.includes(subcustomer)) {
                            ...
                            if (miner.hashrate) {
                                ...
                                subcustomerStats[subcustomer].expectedHashRate += miner.expectedHashRate;
                            }
                            break;
                        }
                    }
                ...
                let fortitudeTable = emailBody.querySelector('table:nth-of-type(2)');
                const rammTable = emailBody.querySelector('table:nth-of-type(3)');
                const bitmainTable = emailBody.querySelector('table:nth-of-type(4)');

                if (generalStatsTable) {
                    const rows = generalStatsTable.rows;
                    rows[1].cells[1].innerText = ``; //`/${subcustomerStats.Fortitude.total+subcustomerStats.RAMM.total+subcustomerStats.Bitmain.total}`;
                }
                if (fortitudeTable) {
                    ...
                    const efficiency = ((subcustomerStats.Fortitude.hashrate / subcustomerStats.Fortitude.expectedHashRate) * 100).toFixed(2);
                    rows[5].cells[1].innerText = `${efficiency}%`;
                }
                if (rammTable) {
                    const rows = rammTable.rows;
                    rows[1].cells[1].innerText = `/${subcustomerStats.RAMM.total}`;
                    let [hash, unit] = convertHashRate(subcustomerStats.RAMM.hashrate);
                    rows[2].cells[1].innerText = `${hash} ${unit}/s`;

                    const efficiency = ((subcustomerStats.RAMM.hashrate / subcustomerStats.RAMM.expectedHashRate) * 100).toFixed(2);
                    rows[5].cells[1].innerText = `${efficiency}%`;
                }
                if (bitmainTable) {
                    const rows = bitmainTable.rows;
                    rows[1].cells[1].innerText = `/${subcustomerStats.Bitmain.total}`;
                    let [hash, unit] = convertHashRate(subcustomerStats.Bitmain.hashrate);
                    rows[2].cells[1].innerText = `${hash} ${unit}/s`;

                    const efficiency = ((subcustomerStats.Bitmain.hashrate / subcustomerStats.Bitmain.expectedHashRate) * 100).toFixed(2);
                    rows[5].cells[1].innerText = `${efficiency}%`;
                }
            });
...
console.log("OptiReport.js loaded successfully");
